...
public class EventRecordTest {
	...
	public void testInputRecordEvent(){
		WebEventRecord record_focus = builder.component("input:text").target(".text").ofType("focus").withValue("check").build();
		webRecorder.append(record_focus);
		WebEventRecord record_blur = builder.component("input:text").target(".text").ofType("blur").withValue("checkout").build();
