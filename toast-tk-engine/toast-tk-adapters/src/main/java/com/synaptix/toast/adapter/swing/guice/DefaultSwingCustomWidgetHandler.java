...
public class DefaultSwingCustomWidgetHandler implements ICustomRequestHandler {
	...
	public String hanldeFixtureCall(
		...
		if(request instanceof CommandRequest) {
			...
			if(target instanceof JLabel) {
				...
				throw new IllegalAccessError("Command not supported for JTable Type :"
					+ ToStringBuilder.reflectionToString(command, ToStringStyle.SHORT_PREFIX_STYLE));
			}
			else {
				LOG.debug("No Handle for swing component: "
					+ ToStringBuilder.reflectionToString(target, ToStringStyle.SHORT_PREFIX_STYLE));
			}
		}
		else {
			LOG.debug("Unhandled swing request type: "
	...
	public String handle(
		...
		CommandRequest command) {
		// JTextComponentFixture tFixture = new
// JTextComponentFixture(FestRobotInstance.getRobot(), textField);
		switch(command.action) {
			case SET :
				textField.setText(command.value);
				break;
	...
	private String handle(
		JTable target,
		final TableCommandRequest command) {
				...
				for(int i = 0; i < tFixture.rowCount(); i++) {
					...
					if(found) {
						if(tcommand.query.resultCol != null) {
							JTableCellFixture cell = tFixture.cell(TableCellByColumnId.row(i).columnId(
								tcommand.query.resultCol));
							cell.select();
							return cell.value();
						}
						else {
				...
				for(int i = 0; i < tFixture.rowCount(); i++) {
					...
					if(found) {
						...
						try {
							cell.select();
							cell.rightClick();
							JPopupMenuFixture pFixture = new JPopupMenuFixture(FestRobotInstance.getRobot(),
								FestRobotInstance.getRobot().findActivePopupMenu());
