...
public class RepositorySetup implements IRepositorySetup{
	...
	public void setTestManager(ITestManager manager){
	...
	@Inject
	public RepositorySetup(Set<IRepositoryTypeParser> typeHandlers, ClassConfigProvider classConfigProvider) {
	...
	private TestResult insertComponent(String componentName, Map<String, String> values, Class<?> entityClass) {
		componentName = FixtureHelper.parseTestString(componentName);
		...
		for (Entry<String, String> entrySet : values.entrySet()) {
			...
			if (propertyClass == null) {
				return new TestResult("Property " + propertyName + " is not configured or not found. Check entity configuration.", ResultKind.ERROR);
			}
	 ...
	 * @return
	 */
	public Object selectEntity(String entityName, String propertyName, String propertyValue) {
	...
	public TestResult addProperty(String testClassName, String testPropertyName, String appPropertyName, String objectType) {
		...
		if (entityConfig != null) {
			if (entityConfig.addProperty(testPropertyName, appPropertyName, objectType)) {
				...
				return new TestResult("The property " + appPropertyName + " does not exist in " + entityConfig.componentClass, ResultKind.ERROR);
			}
		}
		return new TestResult("Class has not be configured.");
	}

	public class TestComponentConfig { // A découper en 2 classes, une pour les entités, une pour les domaines
		 ...
		 * @return
		 */
		public boolean addProperty(String testPropertyName, String appPropertyName, String objectType) {
	...
	public Class<?> getComponentClassFullNameFromTestName(String testName) {
	...
	public String getPropertyName(String className, String testName) {
	...
	public String getPropertyType(String className, String testName) {
		TestComponentConfig testComponentConfig = this.classesConfig.get(testName);
		if (testComponentConfig != null) {
			TestEntityProperty testEntityProperty = testComponentConfig.propertiesMap.get(testName);
			if (testEntityProperty != null) {
	 ...
	 * @param className
	 */
	public TestResult addService(String type, String className) {
		type = FixtureHelper.parseTestString(type);
