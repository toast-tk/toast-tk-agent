...
public class RedPepperMojo extends AbstractMojo {
	...
	public void execute() throws MojoExecutionException {
		...
		getLog().info("Redpepper files will be generated in package " + outputPackage);
		getLog().info("Connecting to -> " + host);
		try{
			...
			for (Driver driver : drivers) {
				List<Sentence> sentences = new ArrayList<RedPepperMojo.Sentence>();
			...
			getLog().info("Redpepper update completed succesfully !");
		}catch(Exception e){
			getLog().error("Redpepper update cancelled !");
			getLog().error(e);
		}
	}

	/**
	 * TODO: loop here over all drivers and write a single file in ./settings/redppeer_descriptor.json
	 * 
	 * @param driver
	 * @param sentences
	 */
	private String writeDriverJson(Driver driver, List<Sentence> sentences) {
		StringBuilder builder = new StringBuilder();
		builder.append("{");
		builder.append("\"").append("driver").append("\"").append(":")
				.append("\"").append(driver.name).append("\"").append(",");
		builder.append("\"").append("type").append("\"").append(":")
		.append("\"").append(driver.type).append("\"").append(",");
		builder.append("\"").append("class").append("\"").append(":")
				.append("\"")
	...
	private List<Sentence> downloadSentences(Driver driver, Client httpClient) {
		List<Sentence> res = new ArrayList<RedPepperMojo.Sentence>();
	...
	private List<Sentence> downloadDynamicSentences(Driver driver,
			Client httpClient) {
		List<Sentence> res = new ArrayList<RedPepperMojo.Sentence>();
		String jsonResponse = RestUtils.getJsonResponseAsString(host + "/loadCtxSentences/" + driver.type + "/" + driver.name, httpClient);
		JSONArray jsonResult;
		try {
			jsonResult = new JSONArray(jsonResponse);
			for (int i = 0; i < jsonResult.length(); i++) {
	...
	private Set<Driver> downloadScenarii(String uri, Client httpClient)
			throws MojoExecutionException {
		String jsonResponse = RestUtils.getJsonResponseAsString(uri, httpClient);
		Set<Driver> drivers = new HashSet<RedPepperMojo.Driver>();
		JSONArray jsonResult;
		try {
			jsonResult = new JSONArray(jsonResponse);
